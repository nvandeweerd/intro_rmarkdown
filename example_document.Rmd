...
```{r, fig.align='center', fig.cap="Project Members"}
knitr::include_graphics("project_members.png")
```

# Note: file management

The default working directory is the folder where the .Rmd file is located. In order
to stay organized, it's good to work in "projects". Each project gets its own
folder with sub-folders for data, reports, presentations etc. When working in 
projects, a very handy package is the **here** package which creates file paths
...
The `lsnp.info.csv` file is located in the `data/` folder. I used **here()** to 
build a path there. 

```{r, echo=FALSE}
lsnp.info <- read.csv(here("data", "lsnp.info.csv"))
